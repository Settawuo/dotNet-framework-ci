...
namespace WBBWebService.Extension
{
    ...
    public class TraceExtension : SoapExtension
    {
            ...
            switch (message.Stage)
            {
                ...
                case SoapMessageStage.AfterSerialize:
                     ...
                     (((System.Web.Services.Protocols.WebClientProtocol)(((System.Web.Services.Protocols.SoapClientMessage)(message)).Client)).Credentials != null))
                    {
                        ...
                        transactionId = ((System.Net.NetworkCredential)(((System.Web.Services.Protocols.WebClientProtocol)(((System.Web.Services.Protocols.SoapClientMessage)(message)).Client)).Credentials)).UserName;

                    }


                    traceStream.Position = 0;
                    WriteToLog(DumpType.Request, message.MethodInfo.Name, transactionId);
                    traceStream.Position = 0;
                    traceStream.CopyNewToOld();
                ...
                case SoapMessageStage.BeforeDeserialize:
                    ...
                    traceStream.SwitchToNewStream();
                    traceStream.CopyOldToNew();
                    WriteToLog(DumpType.Response, message.MethodInfo.Name, transactionId);
                    traceStream.Position = 0;
                    break;
            }
        }
        ...
        private void WriteToLog(DumpType type, string traceable, string transactionId)
        {
            try
            {
                ...
                //  Trace.WriteLine(writer.ToString(), String.Format("{0} {1}", traceable, type));

                if (!string.IsNullOrEmpty(transactionId))
                {
                    Task.Factory.StartNew(SaveDataFromSoapMessage(traceable, writer.ToString(), type, transactionId));

                    //var x = await SaveDataFromSoapMessage(traceable, writer.ToString(), type, transactionId);
                }
        ...
        private static Action SaveDataFromSoapMessage(string methodName, string soapMessage, DumpType action, string transactionId)
        {

            var resultcomPare = string.Empty;

            if (action == DumpType.Response) return null;
            try
            {
                //Verify methodName
            ...
            if (string.IsNullOrEmpty(resultcomPare))
            {
                return null;
            ...
            var outgoingOrderNewCommand = new SaveOutgoingMessageCommand
            {
                ...
                Action = action == DumpType.Request ? ActionType.Insert : ActionType.Update,
                SoapXml = soapMessage,
