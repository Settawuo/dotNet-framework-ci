...
namespace FBBPAYG_ReconcileFBSS_CPE
{
    ...
    public class CredentialHelper
    {
                    ...
                    foreach (var item in date_delete)
                    {
                        ...
                        foreach (var file in files_delete)// วนไฟล์ใน path
                        {
                            ...
                            if (firstModified < resultDate)// เช็คว่าไฟล์ไหนมาก่อน resultDate
                            {
                                ...
                                string result_fileName = fileName.Substring(0, lastUnderscoreIndex + 1);// split ชื่อไฟล์กับวัน
        ...
        public bool WriteFile(string username, string pwd, string domin, string pathName, List<FBBPAYGReconcileFBSSHVRReturn> command)
        {
