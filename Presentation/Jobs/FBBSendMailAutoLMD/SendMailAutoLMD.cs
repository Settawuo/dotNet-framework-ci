...
{
    internal class SendMailAutoLMD
    {
        ...
        private readonly ICommandHandler<EmailTransactionCommand> _EmailTranLog;
        private Stopwatch _timer;

        public SendMailAutoLMD(ILogger logger, IQueryProcessor queryProcessor
            ...
            , ICommandHandler<EmailTransactionCommand> EmailTranLog)
        {
            _Logger = logger;
            ...
            _EmailTranLog = EmailTranLog;
        }

        public void ManagerAsync()
        {
            try
            {
                _Logger.Info("SendMailAutoLMD start.");
                StartWatching();
            }
            catch (Exception er)
            {
                _Logger.Info("exception timer.");
            }
            ...
            try
            {
                ...
                {
                    CON_TYPE = "FBB_SENDMAILAUTOLMD_BATCH",
                    CON_NAME = "PROGRAM_PROCESS"
                };
                ...
                {
                    _Logger.Info("FBBNoDataResendPendingBatch Program process : " + _FbssConfig.DISPLAY_VAL);
                }
        ...
        {
            _timer.Stop();
            _Logger.Info("PAYG start FBBSendMailAutoLMD take : " + _timer.Elapsed);
        }
        private async Task<string> SendMailUpdateOrderStatus(List<LastMileByDistanceOrderReturn> data, APPROVEFMTO aPPROVEFMTO)
        {
            ...
            try
            {
                ...
                {
                    p_USER = "FBB_SENDMAILAUTOLMD_BATCH",
        ...
        private APPROVEFMTO APPROVE_FM_TO()
        {
            try
            {
                ...
                var chkHoliday = false;

                var datePay = IsDayOfWeek(dataDoc.AddDays(30), DayOfWeek.Monday);
                var dateHolidayList = Get_FBSS_CONFIG_TBL_LOV("FBB_SENDMAILAUTOLMD", "NONE_PAID").ToList();
                do
                {
                    var dateHoliday = dateHolidayList
                                       .Where(h => h.VAL1.Equals(datePay.ToString("ddMMyyyy", customCulture)))
                                       .FirstOrDefault();


                    if (dateHoliday != null && dateHoliday.ACTIVEFLAG == "Y")
                    {
                        datePay = datePay.AddDays(1);
                        chkHoliday = true;
                    }
                    else
                    {
        ...
        private List<FbssConfigTBL> Get_FBSS_CONFIG_TBL_LOV(string _CON_TYPE, string _CON_NAME)
        {
            var query = new GetFbssConfigTBLQuery()
            {
                CON_TYPE = _CON_TYPE,
        ...
        private DateTime IsDayOfWeek(DateTime _date, DayOfWeek dow)
        {
            DateTime dateresult = _date;
            bool YesMonday = false;
            while (!YesMonday)
            {
                if (dateresult.DayOfWeek == dow)
                {
                    YesMonday = true;
                }
                else
        ...
        private string GetPeriodDate(DateTime dateFM, DateTime dateTO)
        {
            ...
            var dateThai = $"{date_FM.ToString("dd MMM yy")} - {date_TO.ToString("dd MMM yy")}";
            return dateThai;
                    ...
                    if (groupBySubName.Count > 0)
                    {
                        for (int j = 0; j <= groupBySubName.Count - 1; j++) // แยก subcontect
                        {
                            ...
                            if (groupByOrderStatus.Count > 0)
                            {
                                ...
                                string contents = EmailTemplate(SUBCONTRACT_NAME, sTRING_CS_APPROVE_DATE, strArr, cS_DOC_DATE, cS_PAYMENT_DATE, cS_REMARK);
                                ...
                                if (String.IsNullOrEmpty(chkstatus) == true)
                                {
                                    ...
                                    InsertEmailTranLog(SUBCONTRACT_EMAIL.ToSafeString(), contents.ToSafeString(), fileName + ".xlsx", "SUCCESS", cS_REMARK, "BATCH");
                                    successcount += 1;
                                }
                                else
                                {
                                    emaillogstatus = chkstatus;
                                    InsertEmailTranLog(SUBCONTRACT_EMAIL.ToSafeString(), contents.ToSafeString(), fileName + ".xlsx", "ERROR", cS_REMARK, "BATCH");
                                    errorcount += 1;
        ...
        private async Task<string> sendEmailByAttachFile(string fileName, MemoryStream msExcel, string SUBCONTRACT_NAME
        ...
        private GenerateExcelModel GenerateExcel(DataSet dataToExcel, string fileName)
        {
            ...
            ScmOrderListModelBySendMail scmOrderList = new ScmOrderListModelBySendMail();
                ...
                if (impersonateVar != null)
                {
                    ...
                    try
                    {
                        ...
                        using (var package = new ExcelPackage(excel))
                        {
                            ...
                            scmOrderList.FileData = package.GetAsByteArray();
                        }
                        ...
                        if (SaveFileNY.ToUpper() == "Y")
                        {
                            ...
                            {
                                ms.CopyTo(new MemoryStream(scmOrderList.FileData));
                                ...
                                if (!string.IsNullOrEmpty(user))
                                {
                                    using (var impersonator = new Impersonator(user, ip, pass, false))
                                    {
                                        ...
                                        {
                                            new MemoryStream(scmOrderList.FileData).CopyTo(fs);
                                            fs.Flush();
                    ...
                    catch (Exception exs)
                    {
                        ...
                        return new GenerateExcelModel()
                        {
                            physicalPath = "Error GenExcel LastMile Sendmail" + exs.Message.ToString(),
                            msExcel = null
                        };
            ...
            return new GenerateExcelModel()
            {
                ...
                msExcel = new MemoryStream(scmOrderList.FileData)
            }; ;
        ...
        private void InsertEmailTranLog(string _EMAIL_TO, string _EMAIL_CONTENT, string _EMAIL_ATTACH, string _STATUS, string _STATUS_DESC, string _CREATE_BY)
        {
            try
            {
                var command = new EmailTransactionCommand
                {
                    PROCESS_NAME = "SEND_EMAIL_FIXED_LASTMILE",
                    EMAIL_TO = _EMAIL_TO,
                    EMAIL_CONTENT = _EMAIL_CONTENT,
                    EMAIL_ATTACH = _EMAIL_ATTACH,
                    STATUS = _STATUS,
                ...
                _EmailTranLog.Handle(command);
            }
            catch (Exception ex)
            {
                _Logger.Info("InsertEmailTranLog Exception : " + ex.ToSafeString());
            }
