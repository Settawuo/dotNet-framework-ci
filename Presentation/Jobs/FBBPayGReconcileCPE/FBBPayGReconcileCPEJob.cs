...
namespace FBBPayGReconcileCPE
{
    ...
    public class FBBPayGReconcileCPEJob
    {
        public ILogger _logger;
        private readonly IQueryProcessor _queryProcessor;
        private readonly ICommandHandler<PAYGReconcileCPECommand> _ReconcileCPE;
        private readonly ICommandHandler<SendMailBatchNotificationCommand> _sendMail;
        private string errorMsg = string.Empty;
        private Stopwatch _timer;

        public FBBPayGReconcileCPEJob(
            ILogger logger,
            IQueryProcessor queryProcessor,
            ICommandHandler<PAYGReconcileCPECommand> reconcileCPE,
            ICommandHandler<SendMailBatchNotificationCommand> sendMail)
        {
            _logger = logger;
            _ReconcileCPE = reconcileCPE;
            _sendMail = sendMail;
        ...
        public void ExecuteJob()
        {
            try
            {
                _logger.Info("Start Reconcile CPE.");
                StartWatching();
                var command = new PAYGReconcileCPECommand()
                {

                };

                _ReconcileCPE.Handle(command);
                //command.Return_Code = "1";

                _logger.Info(string.Format("Reconcile CPE : {0}", command.Return_Message));

                StopWatching("Reconcile CPE");
                ...
                if (command.Return_Code.ToSafeDecimal() != 0)
                {
                    if (SendEmailError)
                        ...
                        _logger.Info("Reconcile CPE : Error without Sending an Email.");
                }
            }
            catch (Exception ex)
            {
                StopWatching("Reconcile CPE");
                _logger.Info("Reconcile CPE" + string.Format(" is error on execute : {0}.",
                    ex.GetErrorMessage()));
        ...
        {
            get { return string.Format("[{0}-Batch] FBBPAYGReconcileCPE", ConfigurationManager.AppSettings["ModeSendEmail"].ToSafeString()); }
        }
