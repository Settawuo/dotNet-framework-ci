using FBBInventoryResendPending.CompositionRoot;
using System;
using System.Linq;
    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                var job = Bootstrapper.GetInstance<FBBInventoryResendPendingBatchJob>();

                Console.WriteLine("Auto Resend Pending start");
                var PROCESS_ROLLBACK = job.Get_FBB_CFG_LOV("FBB_INVENTORYRESENDPENDING_BATCH", "ROLLBACK");

                //ถ้าเป็น Y ใช้ method ใหม่ R21.03.2021
                if (PROCESS_ROLLBACK == "Y")
                {
                    var date_div = job.Get_FBSS_CONFIG_TBL_LOV("FBB_INVENTORYRESENDPENDING_BATCH", "DATE_DIV").FirstOrDefault();
                ...
                Console.WriteLine("Auto Resend Pending end");
            }
            catch (Exception ex)
            {
