...
namespace FBBInventoryResendPendingSAPS4
{
    ...
    public class FBBInventoryResendPendingSAPS4Job
    {
        ...
        public FBBInventoryResendPendingSAPS4Job(
            ILogger logger,
        ...
        public void ResendPendingS4()
        {
            try
            {
                var date_start = Get_FBSS_CONFIG_TBL_LOV("FBBInventoryResendPendingSAPS4_Batch", "DATE_START").Where(record => record.ACTIVEFLAG == "Y").FirstOrDefault();
                var date_to = Get_FBSS_CONFIG_TBL_LOV("FBBInventoryResendPendingSAPS4_Batch", "DATE_TO").Where(record => record.ACTIVEFLAG == "Y").FirstOrDefault();
                var date_diff = Get_FBSS_CONFIG_TBL_LOV("FBBInventoryResendPendingSAPS4_Batch", "DATE_DIFF").Where(record => record.ACTIVEFLAG == "Y").FirstOrDefault();
                var date_to_diff = Get_FBSS_CONFIG_TBL_LOV("FBBInventoryResendPendingSAPS4_Batch", "DATE_TO_DIFF").Where(record => record.ACTIVEFLAG == "Y").FirstOrDefault();
                ...
                try
                {
                    ...
                    {
                        con_type = "FBBInventoryResendPendingSAPS4_Batch",
                        con_name = "DATE_START",
                        display_val = "Y",
                        val1 = c_DATE_START,
                        flag = "EQUIP",
                        updated_by = "FBBInventoryResendPendingSAPS4Batch"
                    };
                    _intfLogCommand.Handle(queryUpdateDate);
                    _logger.Info("Update DISPLAY_VAL DATE_START to Y");

                    ////////////////// CALL PKG WBB.PKG_FBBPAYG_FOA_RESEND_ORDER.P_GET_INV_RESEND_PENDING
                    _logger.Info("Call P_GET_INV_RESEND_PENDING");
                    var query = new FBBInventoryResendPendingSAPS4Query()
                    {
                        p_Date_Start = c_DATE_START,
                        p_Date_To = c_DATE_TO
                    };
                    var FBBInventoryResendPendingSAPS4Response = _queryProcessor.Execute(query);
                    

                    
                    if(FBBInventoryResendPendingSAPS4Response.item_json == null|| FBBInventoryResendPendingSAPS4Response.item_json.Count == 0)
                    {
                    ...
                    else
                    {
                        ...
                        foreach (var item in FBBInventoryResendPendingSAPS4Response.item_json)
                        {
                        ...
                        var batch_Size = Get_FBSS_CONFIG_TBL_LOV("FBBInventoryResendPendingSAPS4_Batch", "BATCH_SIZE").Where(record => record.ACTIVEFLAG == "Y").FirstOrDefault();
                        List<string> allItems = FBBInventoryResendPendingSAPS4Response.item_json;
                        int batchSize = batch_Size.VAL1 != null ? batch_Size.VAL1.ToSafeInteger() : 300;
                    ...
                    var updateDateStart_L = new UpdateFbssFOAConfigTblCommand()
                    {
                        ...
                        con_name = "DATE_START",
                        display_val = "N",
                        val1 = c_DATE_START.ToSafeString(),
                        flag = "EQUIP",
                        updated_by = "FBBInventoryResendPendingSAPS4Batch"
                    };
                ...
                {
                    _logger.Info("Exception at FBBInventoryResendPendingSAPS4Job ERROR : " + ex.Message);
                }
            }
            catch (Exception ex)
            {
