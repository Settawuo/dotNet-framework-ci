...
namespace FBBConfig.Controllers
{
    ...
    public partial class LastMileByDistanceController
    {

        #region LastmileBySendMail
        public async Task<string> SendMailUpdateOrderStatus(List<LastMileByDistanceOrderListModel> data
             , List<ScmOrderListModelBySendMail> OrderListModelBySendMail
           )
        {
            string result = "";
            try
        ...
        public async Task SendEmail(List<FBB_access_list> AccNOList, string dataS)
        {
            ...
            try
            {
                ...
                var LovWorkStatus = LovData.Where(l => l.LovValue3 == "SEND_MAIL" && l.LovValue5 == "FIXED_LASTMILE").ToList();
                ...
                searchModel = (LastMileByDistanceModel)TempData.Peek("TempSearchCriteria");
                ...
                List<LastMileByDistanceOrderListModel> SearchResults = (List<LastMileByDistanceOrderListModel>)result.cur;
                ...
                var DataModel = new JavaScriptSerializer().Deserialize<List<ScmOrderListModelBySendMail>>(dataS);
        ...
        private async Task<string> CreateDataToDataTable(List<LastMileByDistanceOrderListModel> data
...
"","","","","","","","","ยอดรวม",string.Format("{0:n2}", sumLastMile),""
        ...
        #region Lastmile Re-Cal-Distance
        ...
        {
            var query = new GetRuleLastMileByDistanceQuery
            {

            };
            var data = _queryProcessor.Execute(query);
            data.Insert(0, new RuleLastMileByDistanceModel { rule_name = "SELECT", ruleid = "ALL", total_price = 0 });
        ...
        {
            var data = Session["TempTotalPrice"] as List<RuleLastMileByDistanceModel>;
        ...
        public JsonResult RecalDistanceByOrder([DataSourceRequest] DataSourceRequest request
            , List<LastMileRecal> LastMileRecalList,
          string AccNO, string new_ruid, string remark)
        {
            string msg = "";
            if (LastMileRecalList != null)
            {
                string ACCNO = string.Empty;
                foreach (var d in LastMileRecalList)
                {
        ...
        private async Task<JsonResult> Call_Subpayment_APIAsync(ModelApiSubpayment body, string username)
        {

            var result = new Response_API_Subpayment();
            var transactionID = DateTime.Now.ToString("yyyyMMddHHmmss") + "0001";
            InterfaceLogPayGCommand log = StartInterface<ModelApiSubpayment>(body, "Call API Subcontract Payment", transactionID, "");
            try
            {
                var api_subpayment_process = LovData.Where(l => l.Type == "FBBPAYG_LMD_SUBPAYMENT" && l.Name == "UI_FLAG_LMD_SUBPAYMENT").FirstOrDefault();
                ...
                if (api_subpayment_process != null)
                {
                    if (api_subpayment_process.ActiveFlag == "Y")
                    {
                        ...
                        var api_subpayment_url = LovData.Where(l => l.Type == "WS_SubcontractPayment" && l.Name.Contains("UpdatedataRecal")).FirstOrDefault();
                        ...
                        {
                            result = JsonConvert.DeserializeObject<Response_API_Subpayment>(response.Content) ?? new Response_API_Subpayment();
                            EndInterface<Response_API_Subpayment>(result, log, transactionID, result.Result_code == "0000" ? "Success" : "Failed", "");
                        }
                        else
                        {
                            EndInterface<Response_API_Subpayment>(result, log, transactionID, "Failed", "");
                        }
                    }
                }
            }
            ...
            {
                _Logger.Info(ex.GetErrorMessage());
                EndInterface<Response_API_Subpayment>(result, log, transactionID, "System Error", ex.GetErrorMessage());
            }
        ...
        public JsonResult RecalAllByOrder([DataSourceRequest] DataSourceRequest request,
          List<LastMileRecal> LastMileRecalList, string AccNO, string Status, string remark, string new_rule_id)
        {
            string msg = "";
            //   int CountStatusNotMatch = 0, CountStatustMatch = 0;
            int CountStatustMatch = 0;
            List<LastMileRecal> ListLastMileRecal = new List<LastMileRecal>();
            ...
            var AccNOList = result.Where(a => GetOrderStatusByUserGroup().Contains(a.ORDER_STATUS) && LastMileRecalList.Select(x => x.ACCESS_NUMBER).Contains(a.ACCESS_NUMBER)).ToList();
            ...
            try
            {
                ...
                var command = new LastMileByDistanceRecalByOrderCommand()
                {
                    p_recal_access_list = ListLastMileRecal,
        ...
        #region RecalByFile
        public JsonResult LastMileByDistanceReCalByFile([DataSourceRequest] DataSourceRequest request, string status,
         string fileName)
        {
            ...
            //  var fileList = new List<LastMileRecal>(); 
            try
            {
        ...
        public Dictionary<string, string> ExecuteRecalByFileReCheck(string status, string fileName)
        {
            ...
            try
            {
                ...
                var linesData = LastMileFileModel.csv.Split(new[] { "\r\n", "\r", "\n" }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var item in linesData)
                {
                    ...
                    //var model = new LastMileRecal
                    //{
                    //    ACCESS_NUMBER = ACCNBR,
                    //    NEW_RULE_ID = NEW_RULEID,
                    //    ORDER_NO = ORD_NO,
                    //    REMARK = REMARK,
                    //};
                    //fileList.Add(model);
                ...
                //fileList.RemoveAt(0);
                //var command = new LastMileByDistanceRecalByFileCommand()
                //{
                //    p_USER = CurrentUser.UserName,
                //    p_STATUS = status,
                //    p_filename = fileName,
                //    p_recal_access_list = fileList
                //};
                //_LastMileByDistanceRecalByFileCommand.Handle(command);
        ...
        public ActionResult ReCalDistanceFapoLastMilefile_Save(IEnumerable<HttpPostedFileBase> RecalfapoLastMilefile)
        {
            if (RecalfapoLastMilefile != null)
            {
                try
                {
                    foreach (var file in RecalfapoLastMilefile)
                    {
        ...
        public ActionResult ReCalDistanceFapoLastMilefile_Remove(string[] fapoLastMilefile)
        {
        ...
        #endregion Lastmile Re-Cal-Distance
