...
{
    public class ReportProblemController : FBBConfigController
    {
        ...
        private string _rptProblemsType = "Problem Type : {0}";
        private string _rptOrder = "Order by : {0} {1}";

        public ReportProblemController(ILogger logger,
        ...
        public ActionResult Report(string dateFrom = "", string dateTo = "")
        {
            ...
            SetViewBagLov("FBBREPORT_PROBLEM_SCREEN", "PROBLEM01");

            Thread.CurrentThread.CurrentCulture = new CultureInfo("en-us");

            var rptModel = new GetReportProblemsQuery();
        ...
        //{
        //    var searchoawcModel = new JavaScriptSerializer().Deserialize<GetReportProblemsQuery>(dataS);
        //    var result = GetDataReportProblem(searchoawcModel);
        ...
        public ActionResult ReportRead([DataSourceRequest] DataSourceRequest request, string dataS = "")
        {
            ...
            {
                var searchrptModel = new JavaScriptSerializer().Deserialize<GetReportProblemsQuery>(dataS);
                ...
                var reportdatas = GetDataReportProblem(searchrptModel);
                request.Sorts = null;
        ...
        public List<ReportProblemsModel> GetDataReportProblem(GetReportProblemsQuery searchrptModel)
        {
            List<ReportProblemsModel> listResult = _queryProcessor.Execute(searchrptModel);
            return listResult;
        }

        public ActionResult Export(string dataS)
        {
            var searchModel = new JavaScriptSerializer().Deserialize<GetExportReportProblemsQuery>(dataS);
            List<ExportReportProblemsModel> listall = _queryProcessor.Execute(searchModel);
            ...
            _rptName = string.Format(_rptName, "Report Problems Report");
            _rptDate = string.Format(_rptDate, DateTime.Now.ToDisplayText());

            if (string.IsNullOrEmpty(searchModel.SortColumnName))
            {
                var lovData = LovData.Where(p => p.Type == "FBBREPORT_PROBLEM_SCREEN" && p.LovValue5 == "PROBLEM01" && p.OrderBy != null).OrderBy(p => p.OrderBy).ToList().FirstOrDefault();
                searchModel.SortColumnName = lovData != null ? lovData.LovValue2 : string.Empty;
            }
            _rptProblemsType = string.Format(_rptProblemsType,
                !string.IsNullOrEmpty(searchModel.ProblemTypeName) ? searchModel.ProblemTypeName : string.Empty);
            ...
            var bytes = GenerateEntityExcel(listall, filename, "PROBLEM01");
        ...
        public byte[] GenerateEntityExcel<T>(List<T> data, string fileName, string lovValue5)
        {
            ...
            var lovDataScreen = LovData.Where(p => p.Type == "FBBREPORT_PROBLEM_SCREEN" && p.LovValue5 == lovValue5 && p.OrderBy != null).OrderBy(p => p.OrderBy).ToList();
        ...
        private byte[] GenerateExcel(DataTable dataToExcel, string excelSheetName, string directoryPath, string fileName)
        {
            ...
            using (var package = new ExcelPackage(newFile))
            {
                ...
                worksheet.Cells["A3,G3"].LoadFromText(_rptProblemsType);
