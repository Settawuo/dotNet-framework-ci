...
$('#globalsearch').remove();
$('.top_search_input').append($.parseHTML(newinput));
...
var imagePath = 'https://search.ais.co.th';
$('#globalsearch').typeahead({
	...
	highlight: true,
    hint: true,
	href:'https://search.ais.co.th/search?q={{results}}',
	group: ["type", "{{group}}"],
...
//							console.log("data.results===== " ,data.results);
		    ...
		    onClickAfter: function (node, a, item, event) {
		    	...
		    	if(item.linkAction === "goTo") {
					...
					window.location = 'https://search.ais.co.th/search?q='+item.name;
...
var postUrl = 'https://search.ais.co.th/api/cms/getquicklink';
$.post( postUrl, function( data ) {
$('.typeahead-container').append($.parseHTML("<div id='quicklink' class='quicklink hide'><span>Quick Links</span><ul></ul></div>"));
	var quicklinkdata = "";
	if(data.results) {
		...
		$('#quicklink > ul').append($.parseHTML(quicklinkdata));
	}
...
function quicklinkClick(link, name) {
	if(link) {
		...
		window.location = 'https://search.ais.co.th/search?q='+name;
