...
namespace WBBBusinessLayer.Extension
{
    ...
    [System.Runtime.InteropServices.GuidAttribute("BE7DA6C5-4025-4277-86EC-5F268D270315")]
    public static class AWAPDirectionHelper
    {
        /// <summary>
...
namespace WBBBusinessLayer.Extension.Security
{
    ...
    /// <summary> 
    /// Summary description for AuthenDecrypt. 
    /// </summary> 
    [ClassInterface(ClassInterfaceType.AutoDual)]
    [Guid("751CE9FE-EDA4-4f16-B9AB-76D532C2D04B")]
    public static class WBBDecrypt
    {
        private static Hashtable keyhastables = new Hashtable();
        ...
        private static string getKey()
        {
            ...
            sKey = keyhastables[indexKey].ToString();
            if (sKey.Length < 1)
            {
                // generic error, if error code unknown... 
                return keyhastables[0].ToString();
            }
            else
            {
                return sKey;
        ...
        private static int getIndexKey()
        {
            CultureInfo ci = new CultureInfo("en-GB");
            int index = 0;
            DateTime dt = DateTime.Now;
            char[] chrDate = dt.ToString("yyyyMMdd", ci).ToCharArray();

            for (int i = 0; i < chrDate.Length; i++)
            {
                index += Int32.Parse(chrDate[i].ToString());
            }
        ...
        {
            keyhastables.Add(0, @"key01");
            keyhastables.Add(1, @"key01");
            keyhastables.Add(2, @"key01");
            keyhastables.Add(3, @"key01");
            keyhastables.Add(4, @"key01");
            keyhastables.Add(5, @"key01");
            keyhastables.Add(6, @"key01");
            keyhastables.Add(7, @"key01");
            keyhastables.Add(8, @"key01");
            keyhastables.Add(9, @"key01");
        }

        public static string textDecrpyt(string Word)
        {
            string key = "";
            string txtEncrypt = "";

            key = getKey();
            EncrypDLL.EncrypDll encypt = new EncrypDLL.EncrypDll();
            txtEncrypt = encypt.Decrypt(Word, key, EncrypDLL.EncrypDll.MD5);

            return txtEncrypt;
        }

        public static string textDecrpyt(string Word, string encryptkey)
        {
            string key = "";
            string txtEncrypt = "";
    ...
    /// <summary> 
    /// Summary description for AuthenEncrypt. 
    /// </summary> 
    [ClassInterface(ClassInterfaceType.AutoDual)]
    [Guid("7ED62B26-B201-424c-B2D4-D1160E3F6F12")]
    public static class WBBEncrypt
    {
        ...
        public static string textEncrpyt(string Word)
        {
            string key = "";
            string txtEncrypt = "";
            ...
            key = getKey();
            EncrypDLL.EncrypDll encypt = new EncrypDLL.EncrypDll();
            txtEncrypt = encypt.Encrypt(Word, key, EncrypDLL.EncrypDll.MD5);

            return txtEncrypt;
        }

        public static string textEncrpyt(string Word, string ecnryptKey)
        {
            string key = "";
            string txtEncrypt = "";
