...
{
    public class InterfaceLogPayGCommandHandler : ICommandHandler<InterfaceLogPayGCommand>
    {
        private readonly ILogger _logger;
        private readonly IWBBUnitOfWork _uow;
        private readonly IEntityRepository<FBB_INTERFACE_LOG_PAYG> _interfaceLogPayG;
        private readonly IEntityRepository<string> _objService;

        public InterfaceLogPayGCommandHandler(ILogger logger,
            IWBBUnitOfWork uow,
            ...
            IEntityRepository<string> objService)
        {
            _logger = logger;
            _uow = uow;
            _interfaceLogPayG = interfaceLogPayG;
            _objService = objService;
        }

        public void Handle(InterfaceLogPayGCommand command)
        {
            var log = InterfaceLogPayGHelper.Log(_interfaceLogPayG, command);
            //_uow.Persist();
            command.OutInterfaceLogId = log.INTERFACE_ID;
        }
    }

    public static class InterfaceLogPayGHelper
    {
        public static FBB_INTERFACE_LOG_PAYG Log(IEntityRepository<FBB_INTERFACE_LOG_PAYG> interfaceLogPayG,
            InterfaceLogPayGCommand command)
        {
            try
            {
                ...
                {
                    #region DirectInsert
                    //var data = new FBB_INTERFACE_LOG();
                    ...
                    var data = new FBB_INTERFACE_LOG_PAYG();
                    data.IN_TRANSACTION_ID = command.IN_TRANSACTION_ID;
                    data.METHOD_NAME = command.METHOD_NAME;
                    data.SERVICE_NAME = command.SERVICE_NAME;
                    data.IN_ID_CARD_NO = command.IN_ID_CARD_NO;
                    ...
                    if (command.IN_TRANSACTION_ID.Contains('.') == true || command.IN_TRANSACTION_ID.Contains(':') == true)
                    {
                        in_transection_id = command.IN_TRANSACTION_ID.Substring(0, 10);
                    }

                    var executeResult = interfaceLogPayG.ExecuteStoredProc("WBB.PKG_FBB_INTERFACE_LOG_PAYG.PROC_INSERT_INTERFACE_LOG",
                    out paramOut,
                    new
                    {
                ...
                else if (command.ActionType == ActionType.Update)
                {
                    ...
                    var log = new FBB_INTERFACE_LOG_PAYG();
                    ...
                    var executeResult = interfaceLogPayG.ExecuteStoredProc("WBB.PKG_FBB_INTERFACE_LOG_PAYG.PROC_UPDATE_INTERFACE_LOG",
                    out paramOut,
                    new
                    {
                ...
                {
                    throw new Exception("InterfaceLogPayGCommand Error");
                }
            }
            catch (Exception ex)
            {
                return new FBB_INTERFACE_LOG_PAYG();
            }
        }
    }
}
