...
{
    public class ReportInstallationCostbyOrderUpdateByFileCommandHandler : ICommandHandler<ReportInstallationCostbyOrderUpdateByFileCommand>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<string> _objService;
        private readonly IEntityRepository<FBB_HISTORY_LOG> _historyLog;
        private readonly IWBBUnitOfWork _uow;
        public ReportInstallationCostbyOrderUpdateByFileCommandHandler(ILogger ILogger, IEntityRepository<string> objService, IEntityRepository<FBB_HISTORY_LOG> historyLog, IWBBUnitOfWork uow)
        {
            _logger = ILogger;
        ...
        public void Handle(ReportInstallationCostbyOrderUpdateByFileCommand command)
        {
            ...
            try
            {
                ...
                {
                    PAYG_UPDATE_FILE_LIST =
                        command.p_file_list.Select(
                            a => new PAYG_UPDATE_FILE_LISTMapping
                            {
                                ACC_NBR = a.ACC_NBR,
                ...
                var p_file_list = OracleCustomTypeUtilities.CreateUDTArrayParameter("p_file_list", "WBB.PAYG_UPDATE_FILE_LIST", packageMappingObjectModel);
                ...
                var executeResult = _objService.ExecuteStoredProc("WBB.PKG_PAYG_INSTALL_COST_RPT.p_update_by_file",
                    out paramOut,
                    new
                    {
                        command.p_INTERFACE,
            ...
            catch (Exception ex)
            {
                ...
                command.ret_code = "-1";
                command.ret_msg = "Error call ReportInstallationCostbyOrderByFileCommandHandler Handler: " + ex.GetErrorMessage();
                if (command.ret_code != "0")
                {
                    historyLog.HISTORY_ID = historyLog.HISTORY_ID;
                    historyLog.ACTION = ActionHistory.ADD.ToString();
        ...
        {
            [OracleArrayMapping()]
            public PAYG_UPDATE_FILE_LISTMapping[] PAYG_UPDATE_FILE_LIST { get; set; }
            ...
            {
                OracleUdt.SetValue(con, udt, 0, PAYG_UPDATE_FILE_LIST);
            }

            public void ToCustomObject(OracleConnection con, object udt)
            {
                PAYG_UPDATE_FILE_LIST = (PAYG_UPDATE_FILE_LISTMapping[])OracleUdt.GetValue(con, udt, 0);
            }
        }

        [OracleCustomTypeMapping("WBB.PAYG_UPDATE_FILE_REC")]
        public class PAYG_UPDATE_FILE_LIST_MappingOracleTypeMappingFactory : IOracleCustomTypeFactory
        {
            public IOracleCustomType CreateObject()
            {
                return new PAYG_UPDATE_FILE_LISTMapping();
            }
        }

        [OracleCustomTypeMapping("WBB.PAYG_UPDATE_FILE_LIST")]
        public class PAYG_UPDATE_FILE_LISTMappingObjectModelFactory : IOracleCustomTypeFactory, IOracleArrayTypeFactory
        {
            ...
            {
                return new PAYG_UPDATE_FILE_LISTMapping[numElems];
            }
        ...
        public class PAYG_UPDATE_FILE_LISTMapping : INullable, IOracleCustomType
        {
            private bool objectIsNull;
            ...
            public static PAYG_UPDATE_FILE_LISTMapping Null
            {
                get
                {
                    var obj = new PAYG_UPDATE_FILE_LISTMapping();
                    obj.objectIsNull = true;
                    return obj;
                }
            }
