...
{
    public class InterfaceLogCommandHandler : ICommandHandler<InterfaceLogCommand>
    {
        ...
        public InterfaceLogCommandHandler(ILogger logger,
            IWBBUnitOfWork uow,
        ...
        public void Handle(InterfaceLogCommand command)
        {
            var log = InterfaceLogHelper.Log(_interfaceLog, command);
            //_uow.Persist();
            command.OutInterfaceLogId = log.INTERFACE_ID;
        }
    }

    public static class InterfaceLogHelper
    {
        public static FBB_INTERFACE_LOG Log(IEntityRepository<FBB_INTERFACE_LOG> interfaceLog,
            InterfaceLogCommand command)
        {
            try
            {
                if (command.ActionType == ActionType.Insert)
                {
                    ...
                    var executeResult = interfaceLog.ExecuteStoredProc("WBB.PKG_FBB_INTERFACE_LOG.PROC_INSERT_INTERFACE_LOG",
                    out paramOut,
                    new
                    {
                ...
                else if (command.ActionType == ActionType.Update)
                {
                    ...
                    var executeResult = interfaceLog.ExecuteStoredProc("WBB.PKG_FBB_INTERFACE_LOG.PROC_UPDATE_INTERFACE_LOG",
                    out paramOut,
                    new
                    {
                ...
                else if (command.ActionType == ActionType.UpdateImage)
                {
                    ...
                    var executeResult = interfaceLog.ExecuteStoredProc("WBB.PKG_FBB_INTERFACE_LOG.PROC_UPD_IMAGE_PATH",
                    out paramOut,
                    new
                    {
                        p_interface_id = command.INTERFACE_NODE.ToSafeString(),
                        p_card_image_path = command.IMAGE_PATH,

                        //// return 
                        p_return_code = ret_code,
                        p_return_message = ret_message,
                        p_return_interface_id = interface_id
                    });
                    return log;
                    #endregion
                }
