...
{
    public class PortAssignmentCommandHandler : ICommandHandler<PortAssignmentCommand>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<string> _objService;

        public PortAssignmentCommandHandler(ILogger logger, IEntityRepository<string> objService)
        {
            _logger = logger;
            _objService = objService;
        }

        public void Handle(PortAssignmentCommand command)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                command.Return_Code = -1;
                command.Return_Desc = "Error call PortAssignment handles : " + ex.GetErrorMessage();
            }
        }
    }
}
