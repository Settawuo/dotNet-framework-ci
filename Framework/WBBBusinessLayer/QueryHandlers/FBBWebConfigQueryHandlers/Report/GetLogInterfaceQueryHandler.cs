...
{
    public class GetLogInterfaceQueryHandler : IQueryHandler<GetLogInterfaceQuery, List<LogInterfaceModel>>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<LogInterfaceModel> _interfaceLog;

        public GetLogInterfaceQueryHandler(ILogger logger, IEntityRepository<LogInterfaceModel> interfaceLog)
        {
            _logger = logger;
            _interfaceLog = interfaceLog;
        }

        public List<LogInterfaceModel> Handle(GetLogInterfaceQuery query)
        {
            ...
            try
            {
                var stringQuery =
                    string.Format(
                        "SELECT * FROM TABLE( PKG_FBB_REPORT_LOG_INTERFACE.Search(iInTransactionId => '{0}', iMethodName => '{1}', iServiceName => '{2}', iInIdCardNo => '{3}', iCreatedDateFrom => '{4}',iCreatedDateTo => '{5}', iSortColumn => '{6}', iAscDesc => '{7}', iPageNo => '{8}', iRecordsPerPage => '{9}'))"
                        , inTransactionId
                        , methodName
                        , serviceName
                        , inIdCardNo
            ...
            catch (Exception ex)
            {
                ...
                query.ReturnCode = "-1";
                query.ReturnDesc = "PKG_FBB_REPORT_LOG_INTERFACE.Search() Error : " + ex.Message;
