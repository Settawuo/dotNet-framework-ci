...
{
    public class GetOfficerInfoQueryHandler : IQueryHandler<GetOfficerInfoQuery, GetEmployeeProfileByPINModel>
    {
        ...
        public GetOfficerInfoQueryHandler(ILogger logger,
            IWBBUnitOfWork uow,
        ...
        public GetEmployeeProfileByPINModel Handle(GetOfficerInfoQuery query)
        {
            InterfaceLogCommand log = null;
            //string URL = "";
            //string X_Token = "N=fbbon90day";
            //string urlParameters = "?Key1=";
            GetEmployeeProfileByPINModel result = new GetEmployeeProfileByPINModel();

            try
            {
                log = InterfaceLogServiceHelper.StartInterfaceLog(_uow, _intfLog, query, query.EMP_CODE.PadLeft(8, '0'), "GetOfficerInfo", "GetOfficerInfoQueryHandler", "", "FBB", "");
                ...
                if (EnableCallService == "Y")
                {
                    ...
                    using (var service = new OMWService.WS_OM_OMService())
                    {
                        ...
                        try
                        {
                            ...
                            GetEmployeeProfileByPINModel EmpData = JsonConvert.DeserializeObject<GetEmployeeProfileByPINModel>(json);
                            if (EmpData.NewDataSet.Permission.MsgDetail == "Success" && EmpData.NewDataSet.Permission.MsgStatus == "000")
                            {
                                if (EmpData.NewDataSet.Table.USERID != null)
                                {
                                    result = EmpData;
                        ...
                        //                    result.result1 = dr2.ItemArray[1].ToSafeString();
                        //                    result.THFirstName = dr2.ItemArray[2].ToSafeString();
                        //                    result.THLastName = dr2.ItemArray[3].ToSafeString();
                        //                    InterfaceLogServiceHelper.EndInterfaceLog(_uow, _intfLog, datas, log, "Success", "", "");
                        //                }
                ...
                else
                {
                    ...
                    if (resultData != null && resultData.Count > 0)
                    {
                        ...
                        result.NewDataSet.Table.USERID = userName;
                        result.NewDataSet.Table.THFIRSTNAME = "";
                        result.NewDataSet.Table.THLASTNAME = "";
                    }
