...
{
    public class InterfaceARCQueryHandler : IQueryHandler<InterfaceARCQuery, List<InterfaceARCSendMailDetailList>>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<InterfaceARCSendMailDetailList> _objService;
        private readonly IWBBUnitOfWork _uow;
        private readonly IEntityRepository<FBB_INTERFACE_LOG> _intfLog;
        private readonly IEntityRepository<FBB_EMAIL_PROCESSING> _emailProcService;

        public InterfaceARCQueryHandler(ILogger logger, IWBBUnitOfWork uow,
            IEntityRepository<InterfaceARCSendMailDetailList> objService,
        ...
        public List<InterfaceARCSendMailDetailList> Handle(InterfaceARCQuery query)
        {
            List<InterfaceARCSendMailDetailList> executeResult = new List<InterfaceARCSendMailDetailList>();

            try
            {
                _logger.Info("InterfaceARCQueryHandler Start");
                ...
                executeResult = _objService.ExecuteReadStoredProc("WBB.PKG_INTERFACE_ARC.P_ARCHIVE_LOG",
                    new
                    {
                        p_type = query.p_type,
                ...
                {
                    _logger.Info("End WBB.PKG_INTERFACE_ARC.P_ARCHIVE_LOG output msg: " + ret_msg);
                    return executeResult;

                }
                else //return -1 error
                {
                    _logger.Info("Error return -1 call service WBB.PKG_INTERFACE_ARC.P_ARCHIVE_LOG output msg: " + ret_msg);
                    return null;
                }
            }
            catch (Exception ex)
            {
                ...
                _logger.Info("Error call service WBB.PKG_INTERFACE_ARC.P_ARCHIVE_LOG" + ex.Message);
                SendMail(executeResult, "Error call service WBB.PKG_INTERFACE_ARC.P_ARCHIVE_LOG" + ex.Message);
                return null;
            }

        }

        private void SendMail(List<InterfaceARCSendMailDetailList> _sendMailDetail, string msgCatch)
        {
            try
            {
                ...
                {
                    var sendMailDetaillist = _sendMailDetail.Select(x => new InterfaceARCSendMailDetailList()
                    {
                        ret_code = x.ret_code,
                ...
                {
                    subject = string.Format("RESULT INTERFACE ARC IS FAILED");
