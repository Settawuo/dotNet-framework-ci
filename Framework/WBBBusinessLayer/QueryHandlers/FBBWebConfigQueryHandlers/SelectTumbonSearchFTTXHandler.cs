...
namespace WBBBusinessLayer.QueryHandlers.FBBWebConfigQueryHandlers
{
    ...
    public class SelectTumbonSearchFTTXHandler : IQueryHandler<SelectTumbonFTTTXQuery, List<DropdownModel>>
    {
        ...
        public SelectTumbonSearchFTTXHandler(ILogger logger, IEntityRepository<FBB_COVERAGE_REGION> FBB_COVERAGE_REGION,
        ...
        public List<DropdownModel> Handle(SelectTumbonFTTTXQuery query)
        {
            ...
            var GROUP_AMPHUR = (from z in _FBB_ZIPCODE.Get()
                                ...
                                }).Distinct().Union(groupIdListZiploID);



            if (query.REGION_CODE != "" && query.PROVINCE != "" && query.AUMPHUR != "")
            {
                GROUP_AMPHURListData = (from z2 in GROUP_AMPHUR where z2.RegionCode == query.REGION_CODE && z2.Province == query.PROVINCE && z2.Amphur == query.AUMPHUR select z2.zipcode_rowid_th).Distinct().ToList();
            }


            var resultUU = (from r in _FBB_COVERAGE_REGION.Get()
                            join z in _FBB_ZIPCODE.Get() on r.ZIPCODE_ROWID_TH equals z.ZIPCODE_ROWID
                            where GROUP_AMPHURListData.Contains(r.ZIPCODE_ROWID_TH) && r.COVERAGE_STATUS == "ON_SITE"
                           && r.ACTIVEFLAG == "Y" && z.LANG_FLAG == "N"
                            group z by z.TUMBON into g
                            orderby g.Key
