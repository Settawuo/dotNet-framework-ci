...
{
    class PAYGEnhanceMatDocQueryHandler : IQueryHandler<PAYGEnhanceMatDocQuery, PAYGTransAirnetListResult>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<PAYGTransAirnetFileList> _objService;

        public PAYGEnhanceMatDocQueryHandler(ILogger logger, IEntityRepository<PAYGTransAirnetFileList> objService)
        {
            _logger = logger;
            _objService = objService;
        }

        public PAYGTransAirnetListResult Handle(PAYGEnhanceMatDocQuery query)
        {
            ...
            try
            {
                _logger.Info("PAYGEnhanceMatDocQueryHandler : Start.");

                var packageMappingObjectModel = new WBBBusinessLayer.QueryHandlers.FBBWebConfigQueryHandlers.PAYGEnhanceTransAirnetQueryHandler.PackageMappingObjectModel();
                packageMappingObjectModel.FBBPAYG_TRANS_AIRNET_LIST =

                        query.f_enchance_list.Select(
                           a => new WBBBusinessLayer.QueryHandlers.FBBWebConfigQueryHandlers.PAYGEnhanceTransAirnetQueryHandler.FBBPAYG_TRANS_AIRNET_LISTMapping()
                           {
            ...
            {
                _logger.Info("PAYGEnhanceMatDocQueryHandler : Error.");
