...
{
    public class SendRegisterBulkCorpPromoMainToSFFQueryHandler : IQueryHandler<SendRegisterBulkCorpPromoMainToSFFQuery, returnPromoMainData>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<PromoMainlist> _objService;

        public SendRegisterBulkCorpPromoMainToSFFQueryHandler(ILogger logger, IEntityRepository<PromoMainlist> objService)
        {
            _logger = logger;
            _objService = objService;
        }
        public returnPromoMainData Handle(SendRegisterBulkCorpPromoMainToSFFQuery query)
        {
            try
            {
                ...
                var executeResult = _objService.ExecuteReadStoredProc("WBB.PKG_FBBBULK_CORP_REGISTER.PROC_LIST_PROMOTION_MAIN",
                            new
                            {
                                ...
                                p_sff_promotion_cur = p_sff_promotion_cur

                            }).ToList();

                returnPromoMainData ResultData = new returnPromoMainData();
                ResultData.ListProMain = executeResult;
                ...
                {
                    _logger.Info("EndPROC_LIST_PROMOTION_MAIN" + out_msg);
                    return ResultData;

                }
                else //return -1 error
                {
                    _logger.Info("Error return -1 call service WBB.PKG_FBBBULK_CORP_REGISTER.PROC_LIST_PROMOTION_MAIN output msg: " + out_msg);
                    return null;
            ...
            catch (Exception ex)
            {
                ...
                _logger.Info("Error call service WBB.PKG_FBBBULK_CORP_REGISTER.PROC_LIST_PROMOTION_MAIN" + ex.Message);
