...
{
    public class SendRegisterBulkCorpToServiceVDSLRouterQueryHandler : IQueryHandler<SendRegisterBulkCorpToServiceVDSLRouterQuery, returnServiceVDSLRouterData>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<listServiceVdslRouter> _objService;

        public SendRegisterBulkCorpToServiceVDSLRouterQueryHandler(ILogger logger, IEntityRepository<listServiceVdslRouter> objService)
        {
            _logger = logger;
            _objService = objService;
        }
        public returnServiceVDSLRouterData Handle(SendRegisterBulkCorpToServiceVDSLRouterQuery query)
        {
            try
            {
                ...
                var executeResult = _objService.ExecuteReadStoredProc("WBB.PKG_FBBBULK_CORP_REGISTER.PROC_LIST_SERVICE_VDSL_ROUTER",
                            new
                            {
                                ...
                                p_list_service_vdsl_router = p_list_service_vdsl_router

                            }).ToList();

                returnServiceVDSLRouterData ResultData = new returnServiceVDSLRouterData();
                ResultData.ServiceVDSLRouter = executeResult;
                ...
                {
                    _logger.Info("EndPROC_LIST_SERVICE_VDSL_ROUTER" + out_msg);
                    return ResultData;

                }
                else //return -1 error
                {
                    _logger.Info("Error return -1 call service WBB.PKG_FBBBULK_CORP_REGISTER.PROC_LIST_SERVICE_VDSL_ROUTER output msg: " + out_msg);
                    return null;
            ...
            catch (Exception ex)
            {
                ...
                _logger.Info("Error call service WBB.PKG_FBBBULK_CORP_REGISTER.PROC_LIST_SERVICE_VDSL_ROUTER" + ex.Message);
