...
{
    public class SelectTumbonHandler : IQueryHandler<SelectTumbonQuery, List<LovModel>>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<FBB_ZIPCODE> _FBB_ZIPCODE;

        public SelectTumbonHandler(ILogger logger, IEntityRepository<FBB_ZIPCODE> FBB_ZIPCODE)
        {
            _logger = logger;
        ...
        public List<LovModel> Handle(SelectTumbonQuery query)
        {
            ...
            {
                return (from r in _FBB_ZIPCODE.Get()
                        where r.LANG_FLAG == query.Lang_Flag && r.STATUS == "A" && r.REGION_CODE == query.REGION_CODE && r.AMPHUR == query.AUMPHUR
                        group r by r.TUMBON into g
                        orderby g.Key
                        select new LovModel
                        {
            ...
            {
                return (from r in _FBB_ZIPCODE.Get()
                        where r.LANG_FLAG == query.Lang_Flag && r.STATUS == "A" && r.REGION_CODE == query.REGION_CODE && r.PROVINCE == query.PROVINCE && r.AMPHUR == query.AUMPHUR
                        group r by r.TUMBON into g
                        orderby g.Key
                        select new LovModel
                        {
