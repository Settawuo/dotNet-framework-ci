...
{
    public class GetBulkNumberforExistingQueryHandler : IQueryHandler<GetBulkNumberforExistingQuery, returnBulkNumber>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<returnBulkNumber> _objService;

        public GetBulkNumberforExistingQueryHandler(ILogger logger, IEntityRepository<returnBulkNumber> objService)
        {
            _logger = logger;
            _objService = objService;
        }
        public returnBulkNumber Handle(GetBulkNumberforExistingQuery query)
        {
            try
            {
                ...
                var executeResult = _objService.ExecuteReadStoredProc("WBB.PKG_FBBBULK_CORP_REGISTER.PROC_BULK_NUMBER",
                            new
                            {
                                p_user = query.p_user,
                ...
                returnBulkNumber Resultdata = new returnBulkNumber();
                ...
                {
                    _logger.Info("EndPROC_BULK_NUMBER " + output_return_message);
                    return Resultdata;

                }
                else //return -1 error
                {
                    _logger.Info("Error return -1 call service WBB.PKG_FBBBULK_CORP_REGISTER.PROC_BULK_NUMBER output msg: " + output_return_message);
                    return null;
            ...
            catch (Exception ex)
            {
                ...
                _logger.Info("Error call service WBB.PKG_FBBBULK_CORP_REGISTER.PROC_BULK_NUMBER" + ex.Message);
