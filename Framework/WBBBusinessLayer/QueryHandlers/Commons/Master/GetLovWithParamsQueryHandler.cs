...
{
    public class GetLovWithParamsQueryHandler : IQueryHandler<GetLovWithParamsQuery, List<LovValueModel>>
    {
        private readonly ILogger _logger;
        private readonly IEntityRepository<FBB_CFG_LOV> _lovService;

        public GetLovWithParamsQueryHandler(ILogger logger, IEntityRepository<FBB_CFG_LOV> lovService)
        {
            _logger = logger;
            _lovService = lovService;
        }

        public List<LovValueModel> Handle(GetLovWithParamsQuery query)
        {
            List<FBB_CFG_LOV> loveList = null;
            if (string.IsNullOrEmpty(query.LovType) && string.IsNullOrEmpty(query.LovName) && string.IsNullOrEmpty(query.LovValue5))
            {
            ...
            else
            {
                loveList = _lovService
                    .Get(lov => (!string.IsNullOrEmpty(query.LovType) && lov.LOV_TYPE.Equals(query.LovType))
                            ...
                            && (string.IsNullOrEmpty(query.LovValue5) || lov.LOV_VAL5.Equals(query.LovValue5))
