...
{
    public class GetFileNameCancelFBSS90DayQueryHandler : IQueryHandler<GetFileNameCancelFBSS90DayQuery, AutoMoveFileBatchModel>
    {
        private readonly ILogger _logger;
        private readonly IAirNetEntityRepository<AutoMoveFileBatchModel> _airnetRepository;

        public GetFileNameCancelFBSS90DayQueryHandler(ILogger logger, IAirNetEntityRepository<AutoMoveFileBatchModel> airnetRepository)
        {
            _logger = logger;
            _airnetRepository = airnetRepository;
        }

        public AutoMoveFileBatchModel Handle(GetFileNameCancelFBSS90DayQuery query)
        {
            ...
            try
            {
                ...
                _logger.Info("Start PKG_FBBOR006.PROC_GET_FILE_CANCEL_FBSS By Day");

                var result = _airnetRepository.ExecuteStoredProcMultipleCursor("AIR_ADMIN.PKG_FBBOR006.PROC_GET_FILE_CANCEL_FBSSBYDAY",
                    new object[]
                    {
                        P_DAY,
                        //return code
                ...
                if (result != null)
                {
                    ...
                    _logger.Info("End PKG_FBBOR006.PROC_GET_FILE_CANCEL_FBSS By Day" + executeResults.RES_MESSAGE);
                }

            }
            catch (Exception ex)
            {
                _logger.Info("Error call PKG_FBBOR006.PROC_GET_FILE_CANCEL_FBSS By Day handles : " + ex.Message);
