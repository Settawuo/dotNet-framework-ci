...
{
    public class CreateOrderNewRegisterWTTxHandler : IQueryHandler<CreateOrderNewRegisterWTTxQuery, CreateOrderNewRegisterWTTxModel>
    {
        private readonly ILogger _logger;
        private readonly IWBBUnitOfWork _uow;
        private readonly IEntityRepository<FBB_INTERFACE_LOG> _intfLog;
        private readonly IQueryHandler<GetWTTXReserveQuery, WTTXReserveModel> _getWTTXReserve;
        ...
        public CreateOrderNewRegisterWTTxHandler(ILogger logger,
            IWBBUnitOfWork uow,
            IEntityRepository<FBB_INTERFACE_LOG> intfLog,
            IQueryHandler<GetWTTXReserveQuery, WTTXReserveModel> getWTTXReserve,
            IQueryHandler<GetConfigReqPaymentQuery, GetConfigReqPaymentModel> getConfigReqPayment,
            IQueryHandler<GetListORDDetailCreateQuery, GetListORDDetailCreateModel> objGetListORDDetail,
            IQueryHandler<GetSaveOrderRespJobQuery, SaveOrderResp> getSaveOrderRespJob,
            ...
            IQueryHandler<PDFDataQuery, PDFData> pdfData,
            ICommandHandler<MailLogCommand> mailLogCommand,
            ICommandHandler<UpdateFileNameCommand> UpdateFileName,
            IQueryHandler<GetFormatFileNameEAPPQuery, FileFormatModel> GetFormatFileNameEAPP,
            ...
            )
        {
            ...
            _uow = uow;
            _intfLog = intfLog;
            _getWTTXReserve = getWTTXReserve;
        ...
        public CreateOrderNewRegisterWTTxModel Handle(CreateOrderNewRegisterWTTxQuery query)
        {
            var isCreateOrder = false;
            InterfaceLogCommand log = null;

            var resultModel = new CreateOrderNewRegisterWTTxModel();
            try
            {
                FullUrlStr = query.FullUrl;
                log = InterfaceLogServiceHelper.StartInterfaceLog(_uow, _intfLog, query, query.txn_id, "CreateOrderNewRegisterWTTx", "CreateOrderNewRegisterWTTxHandler", "", "FBB|" + query.FullUrl, "");
                ...
                if (dataORDDetailCreate != null)
                {
                    ...
                    string MsgSent = "";
                    List<LovValueModel> MsgSentData = GetLovByTypeName("SCREEN", "SMS_MESSAGE_WTTX_PAYMENT_SUCCESS");
                    if (MsgSentData != null && MsgSentData.Count > 0)
                    {
                        if (isThai)
                        {
                            MsgSent = MsgSentData.FirstOrDefault().LovValue1.ToSafeString();
                        }
                        else
                        {
                            MsgSent = MsgSentData.FirstOrDefault().LovValue2.ToSafeString();
                        }
                        // Sent SMS Paid
                        if (CheckSMSFlag(query.OrderId, "Success", dataORDDetailCreate.ODRDetailCustomerList[0].mobile_no, "1", query.FullUrl))
                        {
                            string resultsmsPaid = SendSMS(dataORDDetailCreate.ODRDetailCustomerList[0].contact_mobile_no_1, "", isThai, dataORDDetailCreate.ODRDetailCustomerList[0].relate_non_mobile.ToSafeString(), MsgSent);
                            if (resultsmsPaid == "Success")
                            {
                                UpdateSMSFlag(query.OrderId, "Success", dataORDDetailCreate.ODRDetailCustomerList[0].mobile_no, "1", query.FullUrl);
                            }
                        }
                    }

                    string ReservedId = GetWTTXReserve(query, dataORDDetailCreate.ODRDetailCustomerList[0].mobile_no);
                    ...
                    if (query.payment_transaction_date.ToSafeString() == "")
                    {
                        DateTime paymentTransactionDate = DateTime.Now;
                        query.payment_transaction_date = paymentTransactionDate.ToString("dd/MM/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);
                    }

                    SaveOrderResp saveOrderResp = new SaveOrderResp();
                    _logger.Info("GetSaveOrderResp order_id :" + query.OrderId);
                    saveOrderResp = GetSaveOrderResp(dataORDDetailCreate, query.txn_id, query.payment_transaction_date, query.payment_method, ReservedId);
                    ...
                    if (isCreateOrder)
                    {
                        ...
                        // Add ReservedId
                        customerRowID = RegisterCustomer(dataORDDetailCreate, ReservedId, query.access_mode);
                        ...
                        else
                        {
                            ...
                            GenPDFAndSendEmail(customerRowID, dataORDDetailCreate.ODRDetailCustomerList[0].id_card_no, saveOrderResp.RETURN_ORDER_NO, dataORDDetailCreate.ODRDetailCustomerList[0].contact_mobile_no_1, EmailAddress, isThai, user, pass, ip, Impersonate);
                            ...
                            if (dataORDDetailCreate.ODRDetailPackageList.Count > 0 && isCreateOrder)
                            {
                                ...
                                {
                                    if (!string.IsNullOrEmpty(dataORDDetailCreate.ODRDetailPackageList[i].sff_promotion_code) && dataORDDetailCreate.ODRDetailPackageList[i].package_type == "1")
                                    {
                            ...
                            string MsgSentEnd = "";
                            List<LovValueModel> MsgSentEndData = GetLovByTypeName("SCREEN", "SMS_MESSAGE_WTTX_REGIS_SUCCESS");
                            if (MsgSentEndData != null && MsgSentEndData.Count > 0)
                            {
                                if (isThai)
                                {
                                    MsgSentEnd = MsgSentEndData.FirstOrDefault().LovValue1.ToSafeString();
                                }
                                else
                                {
                                    MsgSentEnd = MsgSentEndData.FirstOrDefault().LovValue2.ToSafeString();
                                }
                                ...
                                {
                                    string resultsmsEnd = SendSMS(dataORDDetailCreate.ODRDetailCustomerList[0].contact_mobile_no_1, mainCode, isThai, dataORDDetailCreate.ODRDetailCustomerList[0].relate_non_mobile.ToSafeString(), MsgSentEnd);
                                    if (resultsmsEnd == "Success")
                                    {
            ...
            catch (Exception ex)
            {
                ...
                _logger.Info("Error call CreateOrderNewRegisterWTTxHandler : " + ex.GetErrorMessage());
            }
            finally
            {
        ...
        private string GetWTTXReserve(CreateOrderNewRegisterWTTxQuery query, string MobileNO)
        {
            string reservedIdSTR = "";
            // get from lov RESERVE_EXPIRE_WTTX
            int reservedExpTime = 24;
            FBB_CFG_LOV loveList = _lovService.Get(lov => lov.ACTIVEFLAG.Equals("Y") && lov.LOV_NAME == "RESERVE_EXPIRE_WTTX").FirstOrDefault();
            if (loveList != null && loveList.LOV_VAL1 != null)
            {
                int.TryParse(loveList.LOV_VAL1, out reservedExpTime);
            }

            DateTime expTime = DateTime.Now.AddHours(reservedExpTime);
            GetWTTXReserveQuery getWTTXReserveQuery = new GetWTTXReserveQuery()
            {
                gridId = query.address_id,
                reservedExpTime = expTime.ToString("dd/MM/yyyy HH:mm:ss"),
                transaction_id = MobileNO
            };

            WTTXReserveModel wttxReserveModel = _getWTTXReserve.Handle(getWTTXReserveQuery);
            if (wttxReserveModel != null)
            {
                reservedIdSTR = wttxReserveModel.reservedId != null ? wttxReserveModel.reservedId : "";
            }
            return reservedIdSTR;
        }

        private string GetConfigReqPayment(CreateOrderNewRegisterWTTxQuery query)
        {
            var payment_method_id = "";

            if (query.payment_method == "Rabbit Line Pay")
            {
                query.Channel = "RLP";
            }
        ...
        private SaveOrderResp GetSaveOrderResp(GetListORDDetailCreateModel model, string RegisPaymentID, string RegisPaymentdate, string RegisPaymentmethod, string ReservedId)
        {
            ...
            if (model.ODRDetailCustomerList != null && model.ODRDetailCustomerList.Count > 0)
            {
                foreach (var item in model.ODRDetailCustomerList)
                {
                    ...
                    query.SALE_STAFF_NAME = item.sale_staff_name.ToSafeString();
                    ...
                    query.RESERVED_PORT_ID = ReservedId.ToSafeString();
                    ...
                    query.NON_MOBILE_NO = item.relate_non_mobile.ToSafeString();
                    query.REGIS_PAYMENT_ID = RegisPaymentID;
                    query.REGIS_PAYMENTDATE = RegisPaymentdate;
                    query.REGIS_PAYMENTMETHOD = RegisPaymentmethod;
                    query.REQUIRE_CS_VERIFY_DOC = item.request_cs_verify_doc.ToSafeString();
                    query.FACERECOG_FLAG = item.facerecog_flag.ToSafeString();
                    query.SPECIAL_ACCOUNT_FLAG = item.special_account_flag.ToSafeString();
                    query.LINE_TEMP_ID = item.line_temp_id.ToSafeString();
                    query.SERVICE_YEAR = item.service_year.ToSafeString();
                }
            }
        ...
        private string RegisterCustomer(GetListORDDetailCreateModel model, string ReservedId, string AccessMode)
        {
        ...
        private string SendSMS(string mobileNo, string mainCode, bool IsThaiCulture, string nonMobileNo, string msgtxt)
        {
            ...
            if (!string.IsNullOrEmpty(nonMobileNo))
            {
                msgtxt = msgtxt.Replace("{non_mobile_no}", nonMobileNo);
            }
        ...
        private bool CheckSMSFlag(string orderId, string status, string ContactMobile, string Option, string fullUrl)
        {
        ...
        private void UpdateSMSFlag(string orderId, string status, string ContactMobile, string Option, string fullUrl)
        {
