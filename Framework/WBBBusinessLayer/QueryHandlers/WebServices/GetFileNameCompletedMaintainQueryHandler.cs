...
{
    public class GetFileNameCompletedMaintainQueryHandler : IQueryHandler<GetFileNameCompletedMaintainQuery, AutoMoveFileBatchModel>
    {
        private readonly ILogger _logger;
        private readonly IAirNetEntityRepository<AutoMoveFileBatchModel> _airnetRepository;

        public GetFileNameCompletedMaintainQueryHandler(ILogger logger, IAirNetEntityRepository<AutoMoveFileBatchModel> airnetRepository)
        {
            _logger = logger;
            _airnetRepository = airnetRepository;
        }

        public AutoMoveFileBatchModel Handle(GetFileNameCompletedMaintainQuery query)
        {
            ...
            try
            {
                ...
                _logger.Info("Start PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_COMPLETED ");

                var result = _airnetRepository.ExecuteStoredProcMultipleCursor("AIR_ADMIN.PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_COMPLETED",
                    new object[]
                    {
                        //return code
                        res_code,
                ...
                if (result != null)
                {
                    ...
                    _logger.Info("End PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_COMPLETED" + executeResults.RES_MESSAGE);
                }

            }
            catch (Exception ex)
            {
                _logger.Info("Error call PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_COMPLETED handles : " + ex.Message);
