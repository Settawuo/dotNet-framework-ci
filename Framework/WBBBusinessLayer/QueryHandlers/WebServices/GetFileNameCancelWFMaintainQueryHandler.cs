...
{
    public class GetFileNameCancelWFMaintainQueryHandler : IQueryHandler<GetFileNameCancelWFMaintainQuery, AutoMoveFileBatchModel>
    {
        private readonly ILogger _logger;
        private readonly IAirNetEntityRepository<AutoMoveFileBatchModel> _airnetRepository;

        public GetFileNameCancelWFMaintainQueryHandler(ILogger logger, IAirNetEntityRepository<AutoMoveFileBatchModel> airnetRepository)
        {
            _logger = logger;
            _airnetRepository = airnetRepository;
        }

        public AutoMoveFileBatchModel Handle(GetFileNameCancelWFMaintainQuery query)
        {
            ...
            try
            {
                ...
                _logger.Info("Start PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_WF ");

                var result = _airnetRepository.ExecuteStoredProcMultipleCursor("AIR_ADMIN.PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_WF",
                    new object[]
                    {
                        //return code
                        res_code,
                ...
                if (result != null)
                {
                    ...
                    _logger.Info("End PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_WF" + executeResults.RES_MESSAGE);
                }

            }
            catch (Exception ex)
            {
                _logger.Info("Error call PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_WF handles : " + ex.Message);
