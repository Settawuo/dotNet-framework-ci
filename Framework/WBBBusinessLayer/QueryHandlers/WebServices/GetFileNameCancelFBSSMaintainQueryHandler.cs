...
{
    public class GetFileNameCancelFBSSMaintainQueryHandler : IQueryHandler<GetFileNameCancelFBSSMaintainQuery, AutoMoveFileBatchModel>
    {
        private readonly ILogger _logger;
        private readonly IAirNetEntityRepository<AutoMoveFileBatchModel> _airnetRepository;

        public GetFileNameCancelFBSSMaintainQueryHandler(ILogger logger, IAirNetEntityRepository<AutoMoveFileBatchModel> airnetRepository)
        {
            _logger = logger;
            _airnetRepository = airnetRepository;
        }

        public AutoMoveFileBatchModel Handle(GetFileNameCancelFBSSMaintainQuery query)
        {
            ...
            try
            {
                ...
                _logger.Info("Start PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_FBSS ");

                var result = _airnetRepository.ExecuteStoredProcMultipleCursor("AIR_ADMIN.PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_FBSS",
                    new object[]
                    {
                        //return code
                        res_code,
                ...
                if (result != null)
                {
                    ...
                    _logger.Info("End PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_FBSS" + executeResults.RES_MESSAGE);
                }

            }
            catch (Exception ex)
            {
                _logger.Info("Error call PKG_FBBOR006_MAINTAIN.PROC_GET_FILE_CANCEL_FBSS handles : " + ex.Message);
